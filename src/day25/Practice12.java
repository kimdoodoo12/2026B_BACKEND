...
class Animal{
    void makeSound(){System.out.println("동물은 웁니다");}
}
...
// 생성자 내부에는 super() 생략시 자동으로 적용된다. 즉, super() 키워드는 부모 생성자 호출 키워드
class Machine{
    Machine(){System.out.println("머신 생성자 실행");}
}
...
// Upcasting으로 triangle이 생성 될 때 부모인 Figure까지 같이 생성되기에 자동 형변환이된다.
...
shape.draw();
// Circle의 인스턴스에서 draw()함수는 부모와 자식과 똑같은 함수가 있지만, 자식인 Circle의 draw()가 오버라이딩 되었기에 그것이 나온다.
...
if (vehicle instanceof Bus){
    ((Bus)vehicle).checkFare(); // Downcasting
}
...
beverage.drinkList.add(new Coke());
beverage.drinkList.add(new Coffee());
for (Beverage data: beverage.drinkList){
    data.drink();
}
...
System.out.println(obj.name); obj.method();
// 타입은 Superclass로 Superclass의 멤버변수 name을 쓰지만 메소드는 오버라이딩된 것을 우선하여 쓰기에 하위 메소드를 쓴다.
